...
def resnet50(num_classes, trainer=None):
    return _resnet(Bottleneck, [3, 4, 6, 3], num_classes, trainer=trainer)

if __name__=='__main__':
    simclr_ckpt = torch.load('/home/wendy.liu/.cache/torch/hub/checkpoints/simclr_imagenet.ckpt')
    from torch.hub import load_state_dict_from_url
    state_dict = load_state_dict_from_url('https://download.pytorch.org/models/resnet50-19c8e357.pth', progress=True)
    # print(a['encoder.layer3.3.bn3.weight'])
    print(type(simclr_ckpt),simclr_ckpt.keys())
    print(type(state_dict),state_dict.keys())
    print(type(simclr_ckpt['state_dict']),simclr_ckpt['state_dict'].keys())
    import collections

    state_dict2 = collections.OrderedDict()
    for key, value in simclr_ckpt['state_dict'].items():
        key=key[8:]
        # cifar resnet
        if key not in ['conv1.weight','fc.weight']:
    ...
    temp=[]
    for i in state_dict2.keys():
        if i in state_dict.keys():
            temp.append(i)
    print(len(temp), len(state_dict))
    cifar_r50=resnet50(num_classes=100,trainer='rebalancing')
    cifar_r50.load_state_dict(state_dict2,strict=False)
